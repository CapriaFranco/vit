...
const TournamentBracket = ({ matches, cycle, onEditMatch, finalFormat }) => {
  // Constantes ajustadas
  const MATCH_HEIGHT = 150 // Altura de cada tarjeta
  const MATCH_GAP = 20 // Espacio entre partidos en primera ronda
  const CONNECTOR_WIDTH = 75 // Ancho de las líneas conectoras
  const ROUND_GAP = 0 // Espacio extra entre rondas
        ...
        {hasNoOpponent ? (
          ...
          <div className="h-full flex flex-col">
            ...
            <div className={`flex items-center justify-between p-3 flex-1 ${
              ...
              <div className="flex-1 min-w-0 pr-2">
                <div className="font-bold text-text-primary text-sm truncate">
                  {match.team1?.name === 'TBD' ? '─────────' : (match.team1?.name || '─────────')}
                </div>
                <div className="text-xs text-text-secondary leading-tight">
                  {match.team1?.course && match.team1?.name !== 'TBD' ? match.team1.course : ''}
                </div>
              </div>
              <div className="flex space-x-1">
                {Array(requiredSets).fill().map((_, i) => {
            ...
            <div className={`flex items-center justify-between p-3 flex-1 ${
              ...
              <div className="flex-1 min-w-0 pr-2">
                <div className="font-bold text-text-primary text-sm truncate">
                  {match.team2?.name === 'TBD' ? '─────────' : (match.team2?.name || '─────────')}
                </div>
                <div className="text-xs text-text-secondary leading-tight">
                  {match.team2?.course && match.team2?.name !== 'TBD' ? match.team2.course : ''}
                </div>
              </div>
              <div className="flex space-x-1">
                {Array(requiredSets).fill().map((_, i) => {
  ...
  return (
    <div className="w-full">
      <div className="tournament-bracket-container overflow-x-auto pb-8">
        <div 
          className="flex min-w-max p-8 relative" 
          style={{ 
            gap: `${CONNECTOR_WIDTH + ROUND_GAP}px`,
            minHeight: `${totalBracketHeight + 100}px` // +100 para títulos y padding
          }}
        >
                      ...
                      if (hasSecondMatch) {
                        ...
                        return (
                          <g key={`connector-${match.id}`}>
                            ...
                            {/* Línea horizontal hacia el siguiente partido (extendida) */}
                            <line
                              x1={midX}
                              y1={nextCenterY}
                        ...
                        // Partido BYE (sin pareja): línea horizontal y vertical hasta donde estaría la conexión
                        return (
                          <g key={`connector-${match.id}`}>
